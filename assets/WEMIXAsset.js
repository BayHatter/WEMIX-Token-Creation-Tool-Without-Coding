...
export default function WEMIXAsset({ provider, account, connectWallet }) {
  ...
  const MAIN_WEBSITE_API = 'https://appsiko.com/api/get-token-data?network=wemix';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.log(`Using user config: wallet=${userWallet}, fee=${userFee} WEMIX`);
      } catch (error) {
  ...
  const handleViewToken = () => {
    if (contractAddress) {
      const etherscanUrl = `https://wemixscan.com/token/${contractAddress}`;
    ...
    <div className="container mx-auto p-6 max-w-3xl bg-background">
      ...
      <div className="mt-6 p-6 bg-muted rounded-xl animate-fadeIn">
        <h2 className="text-2xl font-bold font-sans text-black mb-4 relative">
          How to Create Your Token on the WEMIX3.0 Blockchain
        ...
        <p className="text-black font-bold font-sans leading-relaxed mb-4">
          Follow These Simple Steps To Create Meme Coin At WEMIX3.0 Within 10 - 30 Seconds:
        </p>
        <ol className="list-decimal list-inside text-black font-sans font-bold space-y-2">
          ...
          <li>
            Switch Your MetaMask Network to WEMIX3.0. If The WEMIX3.0 Network Isn’t Showing On Metamask, You Can Add It As Your Custom Network. You Can Visit This Link For Getting RPC, Chain Id etc. Details <a href="https://chainlist.org/chain/1111" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">https://chainlist.org/chain/1111</a>, After Getting These Values You Can Add It As Custom Network.
          </li>
